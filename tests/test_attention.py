...
def test_attention_min_confidence_filters() -> None:
    sched = AttentionScheduler(ior=InhibitionOfReturn(max_memory=0), min_confidence=0.5)
    bbps = [
        _bbp(0, 0, 0, 10, 10, 0.2),
        _bbp(0, 0, 0, 10, 10, 0.49),
        _bbp(0, 0, 0, 10, 10, 0.51),
    ]
    sel = sched.select(bbps)
    assert sel is not None
    assert sel.bbp_index == 2
